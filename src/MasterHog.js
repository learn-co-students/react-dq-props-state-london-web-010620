...
export default class MasterHog extends Component {
	...
	myOffspring = () => {
		return offspring.map((pig, index) => {
			return (
				<BabyHog name={offspring[index].name} hobby={offspring[index].hobby} eyeColor={this.state.eyeColor} />
			);
		});
	};
	...
	render() {
		return (
			<div>
				...
				<ul className="hoglist">{this.myOffspring()}</ul>
			</div>
		);
	}
}
